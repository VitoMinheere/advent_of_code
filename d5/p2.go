...
func createDiagonalVents(coords []Coord, distance int) []Coord {
	start := coords[0]
	end := coords[1]
	...
	if x_diff < 0 && y_diff < 0 {
		// start is smaller than end so work backwards
		for i := 1; i < -y_diff; i++ {
			coords = insert(coords, i, Coord{x: start.x + i, y: start.y + i, has_vent: true, vents: 1})
		...
		// diff is positive so goes from right to left
		for i := 1; i < x_diff; i++ {
			coords = insert(coords, i, Coord{x: start.x - i, y: start.y - i, has_vent: true, vents: 1})
		}
...
func main() {
	...
	for scanner.Scan() {
		...
		if line_coords[0].x == line_coords[1].x {
		...
		} else if line_coords[0].y == line_coords[1].y {
			...
			vents = createDiagonalVents(line_coords, hor_dis)
